...
async def test_create_user(db: Session) -> None:
    ...
    user_in = UserCreate(email=email, password=password)  # type: ignore
...
async def test_check_if_user_is_active_inactive(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, disabled=True)  # type: ignore
    user = await crud.user.create(db, obj_in=user_in)  # type: ignore
    is_active = crud.user.is_active(user)
    assert is_active
...
async def test_check_if_user_is_superuser(db: Session) -> None:
    email = random_email()
    password = random_lower_string()
    user_in = UserCreate(email=email, password=password, is_superuser=True)  # type: ignore
    user = await crud.user.create(db, obj_in=user_in)  # type: ignore
...
async def test_get_user(db: Session) -> None:
    password = random_lower_string()
    username = random_email()
    user_in = UserCreate(email=username, password=password, is_superuser=True)  # type: ignore
    user = await crud.user.create(db, obj_in=user_in)  # type: ignore
    user_2 = await crud.user.get(db, id=user["_id"])  # type: ignore
    assert user_2
    assert user["email"] == user_2["email"]  # type: ignore
